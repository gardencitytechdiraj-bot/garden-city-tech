...
function HomePage({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  return (
    <div className="site-shell">
      ...
      <footer className="site-footer"><div className="container footer-grid"><div><img className="footer-logo" src="/assets/gc-logo-secondary.png" alt="Garden City Tech" width="960" height="455" /><p>Rooted in Nepal. Building technology for the world.</p><address className="footer-contact"><span>Pokhara-7, Kaski</span><a href="https://mail.google.com/mail/?view=cm&amp;fs=1&amp;to=info%40gardencitytech.net" target="_blank" rel="noreferrer" aria-label="Email info@gardencitytech.net in Gmail">info@gardencitytech.net</a></address></div><div className="footer-nav"><p className="overline">Explore</p><button type="button" onClick={() => scrollTo("services")}>Services</button><button type="button" onClick={() => scrollTo("about")}>About</button><button type="button" onClick={() => scrollTo("packages")}>Packages</button></div><div className="footer-nav"><p className="overline">Start here</p><button type="button" onClick={() => openApplication()}>Apply for service</button><button type="button" onClick={() => scrollTo("contact")}>Contact</button><button className="admin-link" type="button" onClick={() => onNavigate("/admin")}>Admin access</button></div></div><div className="container footer-bottom"><span>© 2026 Garden City Tech Pvt. Ltd.</span><span>Technology that grows with you.</span></div></footer>
    </div>
  );
}
